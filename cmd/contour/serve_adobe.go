...
func initCache(clients *k8s.Clients, eh *contour.EventHandler, et *contour.EndpointsTranslator) error {
	...
	// Ingresses
	ingresses, err := client.NetworkingV1beta1().Ingresses("").List(context.TODO(), metav1.ListOptions{})
	if err != nil {
		return err
	}
	iCached := 0
	...
	// IngressRoutes
	irs, err := contourClient.Resource(ingressroutev1.IngressRouteGVR).Namespace("").List(context.TODO(), metav1.ListOptions{})
	if err != nil {
		return err
	}
	irCached := 0
	for i := range irs.Items {
		ir, err := converter.FromUnstructured(&irs.Items[i])
		if err != nil {
			return err
		}
	...
	tlscds, err := contourClient.Resource(ingressroutev1.TLSCertificateDelegationGVR).Namespace("").List(context.TODO(), metav1.ListOptions{})
	if err != nil {
		return err
	}

	for i := range tlscds.Items {
		tlscd, err := converter.FromUnstructured(&tlscds.Items[i])
		if err != nil {
			return err
		}
	...
	// Endpoints
	endpoints, err := client.CoreV1().Endpoints("").List(context.TODO(), metav1.ListOptions{})
	if err != nil {
		return err
	}
